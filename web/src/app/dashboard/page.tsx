    ...
    <AppFrame>
      ...
      <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <Link href="/intake?type=missing" className="card p-5 hover:shadow-md transition group">
          <div className="grid h-12 w-12 place-items-center rounded-xl bg-saffron-100 text-saffron-700 dark:text-saffron-300 mb-3">
            <Search className="h-6 w-6" />
          </div>
          <p className="font-bold text-lg">{t("home.reportMissing")}</p>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t("home.reportMissingSub")}</p>
        </Link>
        <Link href="/intake?type=found" className="card p-5 hover:shadow-md transition">
          <div className="grid h-12 w-12 place-items-center rounded-xl bg-teal-100 text-teal-700 dark:text-teal-300 mb-3">
            <UserPlus className="h-6 w-6" />
          </div>
          <p className="font-bold text-lg">{t("home.reportFound")}</p>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t("home.reportFoundSub")}</p>
        </Link>
        <Link href="/map" className="card p-5 flex flex-row sm:flex-col items-center sm:items-start gap-3 hover:shadow-md transition">
          <div className="grid h-12 w-12 place-items-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 sm:mb-3">
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <Link href="/cases" className="text-sm text-saffron-700 dark:text-saffron-300 inline-flex items-center gap-1">
      ...
      <div className="mt-2 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
        ...
        {recent.length === 0 && <p className="text-sm text-slate-400 dark:text-slate-500 py-6 text-center">{t("common.loading")}</p>}
      </div>
    </AppFrame>
  );
}
